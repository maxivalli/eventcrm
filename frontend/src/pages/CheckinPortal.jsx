...
export default function ClientPortal() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/portal/${token}/guests`)
      .then(res => setExistingGuests(res.data.guests))
      .catch(() => setExistingGuests([]))
  }, [token])
  ...
  const sendChat = async () => {
    ...
    setChatLoading(true)
    try {
      // Si los guests todavía no cargaron, fetchearlos ahora antes de mandar
      let guestsData = existingGuests
      if (guestsData === null) {
        try {
          const gRes = await axios.get(`${API}/api/portal/${token}/guests`)
          guestsData = gRes.data.guests
