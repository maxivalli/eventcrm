  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        borderRadius: 18, padding: 32, width: 520, maxWidth: '90vw',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <div>
            ...
            <input type='number' style={inputStyle} value={form.guests} onChange={e => set('guests', e.target.value)} />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={labelStyle}>Presupuesto (ARS)</label>
            <input type='number' style={inputStyle} value={form.budget} onChange={e => set('budget', e.target.value)} />
          </div>
        </div>
...
function EventDetail({ event, onClose, onEdit }) {
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: '#12121e', border: '1px solid #2a2a40',
        borderRadius: 18, padding: 32, width: 460, maxWidth: '90vw',
      }}>
        ...
        {[
          ...
          { label: 'Presupuesto', value: formatCurrency(event.budget) },
  ...
  return (
    <div>
      ...
      <div style={{ background: '#12121e', border: '1px solid #1e1e30', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: '2fr 1.5fr 1fr 1.5fr 1fr 1fr 80px',
          padding: '12px 20px', borderBottom: '1px solid #1e1e30',
          fontSize: 11, color: '#4a4a6a', textTransform: 'uppercase', letterSpacing: 1,
        }}>
          <span>Evento</span><span>Cliente</span><span>Fecha</span>
          <span>Venue</span><span>Presupuesto</span><span>Estado</span><span></span>
        </div>
      ...
      {/* Modales */}
      {modal === 'new' && (
        <EventForm clients={clients} onSave={handleSave} onClose={() => setModal(null)} />
      )}
      {modal === 'edit' && selected && (
        <EventForm initial={selected} clients={clients} onSave={handleSave} onClose={() => { setModal(null); setSelected(null) }} />
      )}
      {modal === 'detail' && selected && (
        <EventDetail
          event={selected}
          onClose={() => { setModal(null); setSelected(null) }}
          onEdit={(ev) => { setModal('edit'); setSelected(ev) }}
        />
      )}
    </div>
  )
