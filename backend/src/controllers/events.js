...
exports.getAll = async (req, res) => {
  try {
    const events = await prisma.event.findMany({
      orderBy: { date: 'asc' },
      include: { client: { select: { id: true, name: true, phone: true } }, _count: { select: { files: true, eventGuests: true } } }
    })
    res.json(events)
  } catch (e) {
...
exports.create = async (req, res) => {
  try {
    const { name, clientId, date, time, venue, type, status, guests, budget, ticketPrice, dietaryOptions } = req.body
    ...
    const event = await prisma.event.create({
      data: { name: name.trim(), venue: venue.trim(), type, status, guests: Number(guests), budget: Number(budget), ticketPrice: ticketPrice ? Number(ticketPrice) : null, date: normalDate, time: time || null, dietaryOptions: dietaryOptions ? JSON.stringify(dietaryOptions) : null, client: { connect: { id: Number(clientId) } } },
